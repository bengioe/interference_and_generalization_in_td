...
def run_exp(meta_seed, nhid, nlayers, n_train_seeds):
    ...
    if 1:
        model = extend(model)
    opt = torch.optim.Adam(model.parameters(), 1e-4, eps=1e-2)#, weight_decay=1e-5)
    #opt = torch.optim.SGD(model.parameters(), 1e-3, momentum=0.99)
    ...
    xents = []
    rews = []
    for i in tqdm(range(200001), smoothing=0):
        ...
        if not i % 5000:
            ...
            print(train_perf[-2:], test_perf[-2:], np.mean(ep_rewards[-50:]), len(ep_rewards), nups, np.mean(xents[-1000:]))
            if 1:
                t1 = time.time()
                s = rp_s[:128]
                if 1:
        ...
        if done:
            ...
            rews = np.cumsum(rews[::-1])[::-1] + 0
            rp_g[ep_start:rp_idx] = torch.tensor(rews).to(device)
            rews = []
            ep_rewards.append(ep_reward)
            ep_reward = 0
            obs = env.reset(np.random.randint(0, n_train_seeds))
            if rp_idx > 32 or True:
                nups += 1
                rp_at = rp_idx
                rp_idx = 0
                rp_fill = 0
                ...
                xents.append((
                    (cls_pi * torch.log(cls_pi)).sum(1).mean() +
                    (mov_pi * torch.log(mov_pi)).sum(1).mean()).item())
                opt.step()
                opt.zero_grad()
            ep_start = rp_idx
...
def main2():
    cfgs = []
    for nhid in [16,8,16,32]:#
        for nlayers in [1,0,1,2]:
            for n_train_seeds in [50000, 20, 100, 500, 1000, 5000, 10000, 50000]:#[4,8,16,32,64,128]:
                for meta_seed in [1,]:
                    cfg = {'nhid': nhid,
                           'nlayers': nlayers,
                           'n_train_seeds': n_train_seeds,
                           'meta_seed': meta_seed,
                           'what':'cifar-reinforce-2'}
                    cfgs.append(cfg)

...
parser.add_argument("--to_test_prob", default=0.1, type=float)
...
def main(argv):
  ...
  for it in range(start_step, num_iterations):
    ...
    *sample, idx = replay_buffer.sample(mbsize)
    ...
    if target == "self":
      ...
      target_ws = past_theta[np.random.randint(0, len(past_theta))]
  ...
  with open(f'results/test_set_{run}.pkl', 'wb') as f:

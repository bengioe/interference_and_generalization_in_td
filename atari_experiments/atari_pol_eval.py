...
def td_interf_grad_terms(A, B, Qf):
  ...
  g_ApB = cross_interferences(lambda: Qf(A.sp).max(1).values,
                              lambda: Qf(B.s)[np.arange(len(B.a)), B.a.long()], theta, t=True)
  g_BpB = cross_interferences_diag(lambda: Qf(B.sp).max(1).values,
                                   lambda: Qf(B.s)[np.arange(len(B.a)), B.a.long()], theta, t=True)

  term0 = delta_B[None, :] ** 2 * g_AB * (g_AB - 0.99 * g_ApB)
  term1 = delta_A[:, None] * delta_B[None, :] * g_AB * (g_BB[None, :] - 0.99 * g_BpB[None, :])
  term2 = delta_A[:, None] * delta_B[None, :]**2

  del g_AB, g_BB, g_ApB, g_BpB
